...
if (token) _axiosDefault.default.get("https://todoo.5xcamp.us/todos", {
    ...
    data.todos.forEach((e)=>{
        todolist1.insertAdjacentHTML("beforeend", `<li data-id="${e.id}"><button>X</button>${e.content}</li>`);
    });
});
...
loginForm.addEventListener("submit", (e)=>{
    ...
    _axiosDefault.default.post("https://todoo.5xcamp.us/users/sign_in", userData).then((data)=>{
        const Showtoken = document.querySelector(".token");
        Showtoken.insertAdjacentHTML("beforeend", `<p>${data.headers.authorization}</p>`);
        const token1 = data.headers.authorization;
...
document.querySelector("#logout_form").addEventListener("submit", (e)=>{
    e.preventDefault();
    const token3 = localStorage.getItem("todo-token");
...
addForm.addEventListener("submit", (e)=>{
    e.preventDefault();
    const token4 = localStorage.getItem("todo-token");
    ...
    _axiosDefault.default.post("https://todoo.5xcamp.us/todos", todoData, {
        ...
        todolist2.insertAdjacentHTML("afterbegin", `<li data-id="${data.id}"><button>X</button>${data.content}</li>`);
    });
});
//Delete
const todolist = document.querySelector("#todolist");
todolist.addEventListener("click", (e)=>{
    const token5 = localStorage.getItem("todo-token");
...
//# sourceMappingURL=1.5baa4167.js.map
